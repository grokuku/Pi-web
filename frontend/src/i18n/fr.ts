...
export const fr = {
  ...
  preview: {
    ...
    restore: "Quitter le plein écran",
    popupMode: "Mode popup : les prochaines previews s'ouvriront dans une fenêtre séparée",
  ...
  modelLibrary: {
    ...
    overrideYes: "Oui",
    overrideNo: "Non",
    capabilitiesHelp: "Auto = détection automatique · Oui/Non = forcé manuellement (utile quand la détection échoue, ex. modèles GLM avec vision)",
  ...
  gitPanel: {
    ...
    // ── Ligne compacte / détail (option A : dépliage auto selon l'état git) ──
    ...
    pushTitle: "Indexer tout → commit → push",
    ahead: (n: number) => `${n} en avance`,
    behind: (n: number) => `${n} en retard`,
    modified: (n: number) => `${n} modifié${n > 1 ? "s" : ""}`,
    created: (n: number) => `${n} nouveau${n > 1 ? "x" : ""}`,
    deleted: (n: number) => `${n} supprimé${n > 1 ? "s" : ""}`,
    staged: (n: number) => `${n} indexé${n > 1 ? "s" : ""}`,
    conflicts: (n: number) => `${n} conflit${n > 1 ? "s" : ""}`,
    filesMore: (n: number) => `+${n} autres fichiers`,
    dirEmpty: "Dossier vide — prêt à cloner",
    dirNotEmpty: "Dossier non vide — clone impossible. Initialisez git puis ajoutez le dépôt distant.",
    cloneRepo: "Cloner le dépôt",
    gitInitRemote: "git init + ajouter le remote",
    timeNow: "à l'instant",
    timeMin: (n: number) => `il y a ${n} min`,
    timeHour: (n: number) => `il y a ${n} h`,
    timeDay: (n: number) => `il y a ${n} j`,
  },
  ...
  // ── Mémoire (Lot M3) ──
  memory: {
    globalSectionTitle: "MÉMOIRE GLOBALE",
    globalSectionDesc: "Profil utilisateur : préférences et conventions valables dans tous les projets.",
    projectSectionTitle: "MÉMOIRE DU PROJET",
    projectSectionDesc: "Décisions et patterns du projet actif, réinjectés dans ses sessions.",
    noActiveProject: "Aucun projet actif — sélectionnez un projet pour gérer sa mémoire.",
    loading: "Chargement…",
    loadError: "Échec du chargement des mémoires",
    empty: "Aucune mémoire pour l'instant.",
    add: "Ajouter",
    edit: "Modifier",
    delete: "Supprimer",
    cancel: "Annuler",
    save: "ENREGISTRER",
    saving: "ENREGISTREMENT…",
    formNewTitle: "NOUVELLE MÉMOIRE",
    formEditTitle: "MODIFIER LA MÉMOIRE",
    titleLabel: "Titre",
    contentLabel: "Contenu",
    typeLabel: "Type",
    tagsLabel: "Tags",
    tagsHint: "séparés par des virgules",
    placeholderTitle: "Ex : Utilise pnpm plutôt que npm",
    placeholderContent: "Contenu détaillé de la mémoire…",
    placeholderTags: "build, docker, tests",
    typePreference: "Préférence",
    typeDecision: "Décision",
    typePattern: "Pattern",
    errTitleRequired: "Le titre est requis",
    errContentRequired: "Le contenu est requis",
    updatedLabel: "Modifiée",
    entriesCount: (n: number) => `${n} mémoire${n > 1 ? "s" : ""}`,
    confirmDelete: (title: string) => `Supprimer « ${title} » ?`,
    confirmYes: "Confirmer",
    feedbackSaved: "✓ Mémoire enregistrée",
    feedbackDeleted: "✓ Mémoire supprimée",
    errSave: "Échec de l'enregistrement de la mémoire",
    errDelete: "Échec de la suppression de la mémoire",
    warnRenameDuplicate: "Mémoire enregistrée, mais l'ancienne entrée n'a pas pu être supprimée : elle subsiste en doublon (nettoyable manuellement).",
