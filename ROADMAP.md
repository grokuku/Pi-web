...
- **[FIXED 2026-06-15] État UI périmé + fuite inter-projets au switch pendant un streaming** — Trois bugs corrélés corrigés :

  **1. `pi_event` pour le projet courant ne synchronisait pas le store `chatHistory`** — Seulement le React state était mis à jour, le store restait figé jusqu'au prochain debounce (500ms) ou switch. Si `pi_history` arrivait dans cette fenêtre, il écrasait le store avec des données finalisées qui ne contiennent PAS le message streaming (le SDK Pi ne commit que sur `message_end`). Résultat : l'assistantId stocké (`assistantIdRef`) pointait vers un message qui n'était plus dans le store, et les `text_delta` suivants étaient silencieusement ignorés (`updateLast` échouait car les IDs ne matchaient plus). Fix : le handler `pi_event` pour le projet courant fait maintenant aussi `chatHistory.saveMessagesFor(result.messages, pid)` + `setAssistantIdFor(pid, result.assistantId)` en synchronisé immédiatement dans le `setMessages(prev => ...)`.

  **2. `pi_history` écrasait le store sans protection contre le streaming en cours** — Pour les projets non-actifs, `saveMessagesFor(display, pid)` écrasait aveuglément le store avec des données backend finalisées (sans le message streaming). Pour le projet courant, `handleHistory()` faisait de même. Dans les deux cas, le streaming était perdu et les `pi_event` suivants ne trouvaient plus l'assistantId dans le store écrasé. Fix : avant d'écrire dans le store, vérifier `existing.some(m => m._streaming)` — si oui, ignorer `pi_history` (le handler `pi_event` est la source de vérité pour le streaming actif).

  **3. Merge localStorage dans `pi_history` non dédupliqué** — Le merge `localStorage` dans le handler `pi_history` fusionnait les messages locaux qui n'étaient pas dans la réponse backend. Comme l'ID local (`Date.now()`) diffère de l'ID serveur, le `Set` ne détectait pas les doublons. Fix : suppression du merge localStorage dans `pi_history` car le store est maintenant la source de vérité synchronisée en continu par `pi_event`.
