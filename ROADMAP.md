...
- **[FIXED 2026-06-15] État UI périmé au switch de projet pendant un streaming** — Le pi_event handler dans ChatView rejetaient silencieusement les events pour les projets non-actifs (`if (msg.projectId !== projectId) return`). Résultat : les text_delta/toolcall_* du streaming d'un projet étaient perdus définitivement. Au retour, `chatHistory.getMessages()` retournait l'état figé au moment du switch, et `currentAssistantIdRef` était `null` donc les updates suivants `updateLastMsg` étaient ignorés. Le LLM lui-même fonctionnait correctement côté backend. Une première tentative via fetch REST `/api/sessions/:id/history` était incorrecte car le SDK Pi ne commit les messages dans `state.session.messages` que sur `message_end` (pas pendant le streaming). **Fix :** extraction du handler pi_event en fonction pure `applyPiEvent()`, ajout d'un `assistantIdRef` Map par projet dans `useChatHistory`, et routing des pi_event vers le stockage du bon projet (même non-actif) pour ne jamais perdre les events. Au switch, on restore depuis `chatHistory` (qui est à jour) et on réconcilie `currentAssistantId`. Fichiers : `ChatView.tsx`, `useChatHistory.ts`.
