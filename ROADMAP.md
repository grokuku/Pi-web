...
- **Constat vérifié (2026-09-19) :** le handshake WebSocket **passe par le forward_auth Authentik**. Testé depuis le conteneur : `wss://pi.holaf.fr/` **et** `wss://pi.holaf.fr/ws` renvoient `HTTP/2 302` vers `authentik.holaf.fr/application/o/authorize/` sans session valide. Contrairement à l'hypothèse ci-dessus, le WS n'est donc **PAS** un tunnel direct Caddy→Pi-Web. Conséquence majeure : toute invalidation de session Authentik bloque le WS → le frontend ne peut plus envoyer `pi_start` → **aucune session n'est créée** (« aucune session active » quel que soit le projet, messages seulement mis en file, aucun log d'erreur backend). Symptôme trompeur : on croit à un bug Pi-Web alors que le backend est sain. Deux pistes prioritaires : (1) cible de l'outpost/proxy provider après un rebuild (l'IP du conteneur change : elle était `172.19.0.27` / hostname `ec8d5cbef23c` le 2026-09-19) — redémarrer l'outpost après chaque rebuild de pi-web ; (2) durée de validité du cookie `authentik_proxy_*` (`Max-Age=86401`, soit 24 h).
- **Robustesse côté Pi-Web (2026-09-19) :** `pi_start` est désormais rejoué (BUG-83), donc l'application se rétablit automatiquement dès que le WS repasse — mais elle reste inutilisable tant qu'Authentik refuse le handshake.
...
| 81 | 🔴 | CBM jamais installé sur un Pi-Web neuf — l'installeur upstream ne connaît plus `--ui` (échec systématique) et l'erreur était avalée (`\| tail -3 \|\| true`) ; binaire non persistant (`~/.local/bin` hors volume) — installation dans `/app/.data/bin` (volume) + `CBM_BIN_PATH` exporté, échec désormais visible, téléchargement en 2 temps (détection d'erreur curl) | 2026-09-19 |
| 82 | 🔴 | `/api/cbm/status` annonçait `installed:false` même binaire installé et serveur actif — la sous-commande `version` de 0.11.0 prend ~7 s et n'écrit rien sur stdout (timeout execSync 5 s) → modal « Graphe 3D » : « Serveur non démarré. Lancez une session Pi » pour **tous** les projets — interrogation via `--version` (≈25 ms) + `installed` découplé de la lecture de version | 2026-09-19 |
| 83 | 🔴 | Session jamais créée après une coupure WS (« aucune session active » quel que soit le projet) — `pi_start` était **volontairement exclu** de la file d'attente et n'était rejoué nulle part (`_ws_reconnect` ne renvoyait que `pi_history_request`) → `pi_start` mis en file + réémis à la reconnexion si la session manque | 2026-09-19 |
