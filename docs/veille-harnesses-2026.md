...
| **Limiteur de concurrence** | Deux pools : **LLM slots** limités **par provider** (avec override `providerMaxLLMSlots[providerId]`) et **agent slots** globaux (sessions Pi simultanées, RAM). Files par provider, timeout de file 60 s, `slotKey` unique par appel (fix BUG-59 : les agents harness ne partagent plus le slot de la session principale). | `backend/src/pi/concurrency.ts` |
